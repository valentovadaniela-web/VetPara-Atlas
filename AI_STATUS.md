...
**Branch:** develop  
**Verzia projektu:** v11 (po vyriešení hamburger menu a loga)

---

## 1. SÚHRN AKTUÁLNEHO STAVU (2026-08-17)

Projekt je funkčný. Milestone 2 (Vizuálny redizajn) je dokončený. Databáza pre psa (38 záznamov) je naimportovaná, ošetrená a funkčná. Všetky aktuálne blokujúce UI problémy boli vyriešené.
...
- ✅ **Aktívny stav v menu:** Pri kliknutí na odkaz sa správne zobrazí biely text s modrým podčiarknutím (vďaka JavaScriptu v `main.js` a CSS v `layout.css`).

### 1.2 Čo bolo dokončené v predchádzajúcich fázach
...
### 2.1 Priorita č. 1: Vyčistenie architektúry
- **`Navbar.js` (mŕtvy kód):** Súbor `src/components/Navbar.js` nie je nikde importovaný (header je natvrdo v `index.html`).
  - *Akcia:* Presunúť tento súbor do priečinka `_archive/` podľa precedensu z `11_SESSION_LOG.md`.
- **CSS čistenie:** V `src/css/layout.css` sa nachádza duplicitné pravidlo `.site-header { background-color: var(--color-bg-header); }` (dvakrát po sebe). 
  - *Akcia:* Odstrániť duplicitný blok.
...
- **Gallery page** (momentálne placeholder).
- **Expert page** (diagnostický systém – momentálne placeholder).
...
- **`src/css/layout.css`** – Štýly pre header, navigáciu a responzivitu (hamburger opravený).
...
- **`src/app/App.js`** – Hlavná logika aplikácie (routing, bindovanie theme toggle).
- **`src/pages/AtlasPage.js`** – Logika pre filtre, vykreslenie zoznamu a detailov.
- **`src/js/main.js`** – Entry point aplikácie (obsahuje JS na prepínanie `.active` v menu).
- **`database/dog.migrated.json`** – Finálna databáza pre psa (38 záznamov).
